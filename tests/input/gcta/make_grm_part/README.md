...
## Overview
This directory contains both **input** and **output** files for the `MAKE_GRM_PART` process test, which calculates partitioned Genomic Relationship Matrices (GRM) from PLINK2 genotype data.

## Files

### Input Files
- **gcta_grm.mpfile** - Merged mpfile specification for genome-wide GRM computation
  - **Source**: Copied from `merge_mpfiles/` directory (output from MERGE_MPFILES)
  - **Format**: Lines with: filename pgen psam pvar
  - **Used by**: Both test cases

- **snp_group1.txt** - High LD SNPs for SNP group filtering
  - **Source**: Copied from `merge_snp_groups/` directory (output from MERGE_SNP_GROUPS)
  - **Contains**: Merged SNP IDs from chr01 + chr02 (high LD quartile)
  - **Used by**: Test 2 (SNP group filtering test)
...
## Data Source
- **Input Files**:
  - `gcta_grm.mpfile` copied from `tests/input/gcta/merge_mpfiles/`
  - `snp_group1.txt` copied from `tests/input/gcta/merge_snp_groups/`
  - Base genotype data from `tests/input/gcta/make_grm_part/chr*.vcf.{pgen,psam,pvar}` (shared test data)
- **Output Files**: Generated by running `nf-test test tests/modules/local/gcta/make_grm_part.nf.test`
- **Test Data**: Self-contained in this directory for `make_grm_part.nf.test`
